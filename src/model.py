...
class Model(object):
    ...
    def fit(self, X, y):
        ...
        try:
            ...
            self.model = GPy.models.SparseGPRegression(X,y,num_inducing=50)
            self.model.kern.lengthscale = 6
            print "Got the model"
            #print self.model
            #self.model.optimize()
            return True
    ...
    def predict(self, X, return_std=True, return_cov=False): # TODO according to version.
        ...
        if self.model:#self.input.size > 0 and self.observations.size > 0:
            ...
            try:
                #print "Calling GPy predict"
                res, res_var = self.model.predict(numpy.array(X))
                #print "After GPy predict"
            except:
                traceback.print_exc(file=sys.stdout)
                print "Exception while predicing with GPy Model"
