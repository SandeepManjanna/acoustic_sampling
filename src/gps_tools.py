...
############## Conversion of GPS. ##############
def convert_gps_to_utm(lat, lon):#, alt=nan):
...
def get_zone_band(lat, lon):
    """Get the zone and band of the GPS coordinates.
    Args:
        lat: latitude.
        lon: longitude.
    Returns:
        (zone, band) tuple
    """
    return geodesy.utm.gridZone(lat, lon)

def set_grid(utm_point, width, height, spacing, orientation=0, zone=0, band='', test=False):
    ...
    if test:
        with open("test_mtl.csv", 'w') as test_file:
            for i in range(len(grid)):
                for j in range(len(grid[i])):
                    #zone, band = 11, 'N'
                    #zone, band = 13, 'S' # for Durango
                    #zone, band = 18, 'T' # for Mtl
...
def main():
    ...
    #utm_point = convert_gps_to_utm(37.23791, -107.90921) #Durango
    #utm_point = convert_gps_to_utm(45.50754, -73.57252) # MTL
    #gridzone = get_zone_band(45.50754, -73.57252)
    utm_point = convert_gps_to_utm(45.50817, -73.56826) # MTL Small
    gridzone = get_zone_band(45.50817, -73.56826)
    zone = gridzone[0]
    band = gridzone[1]
    # positive angle: counterclockwise
    width, height, spacing, orientation = 180, 280, 30.0, -(numpy.pi/6)

    set_grid(utm_point, width, height, spacing, orientation, zone, band, test=True)
